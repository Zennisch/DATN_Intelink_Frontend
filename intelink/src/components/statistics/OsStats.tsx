...
export default function OsStats({ shortCode }: Props) {
    const { getOsStats } = useStatistics();
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const result = await getOsStats(shortCode);
                setData(result);
            } catch (err) {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
            <div className="p-6 border-b border-slate-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h3 className="text-lg font-bold text-slate-800">Operating System Statistics</h3>
                        <p className="text-sm text-slate-500">Traffic sources by operating system.</p>
                    </div>
